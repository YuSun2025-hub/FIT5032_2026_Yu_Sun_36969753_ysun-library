...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>Our #1 seller:</p>
      <ul>
        <li v-for="books in bookstores.topSellers" :key="books">
          {{ books }}
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{ 'highlight-orwell': author.name === 'George Orwell' }"
          :style="author.name === 'George Orwell' ? { fontWeight: 'bold', fontSize: '1.2rem' } : {}"
        >
          {{ author.name }} (born {{ author.birthYear }})
          <span v-if="author.name === 'George Orwell'">⭐</span>
        </li>
      </ul>
